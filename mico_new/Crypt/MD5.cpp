...
//by houjiemin
void MD5::input(const char *in, size_t len)
{
    //byte in_tmp[100] = {0};
    //(in_tmp, in, len);
    update(in, len);
...
//by houjiemin
void MD5::output(char* out)
{
    digest();
    memcpy(out, _digest, 16);
...
void MD5::bytesToString(const byte *input, char* output)
{
...
void MD5::stringTobytes(const char *input, byte* output)
{
    for(size_t i = 0; i < 16; i++) {
        int a = charToHex(input[i * 2]);
        int b = charToHex(input[i * 2 + 1]);
