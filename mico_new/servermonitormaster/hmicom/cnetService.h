...
** 说    明:  台达CNET协议多协议平台头文件，定义一个CNET协议的类
** 版    本:  【可选】
** 创建日期:  2012.06.05
** 创 建 人:  陶健军
** 修改信息： 【可选】
**         修改人    修改日期       修改内容**
*********************************************************************/
#ifndef CNET_SERVICE_H
#define CNET_SERVICE_H
...
** 修改信息： 【若修改过则必需注明】**
 **************************************************************/
class CnetService : public CommService
{
    Q_OBJECT
public:
    CnetService( struct CnetConnectCfg& connectCfg );
    ~CnetService(); //lvyong
    ...
    int addConnect( struct CnetConnectCfg& connectCfg );

    bool updateWriteReq( struct CnetConnect* connect );
    bool updateReadReq( struct CnetConnect* connect );
    void combineReadReq( struct CnetConnect* connect );

    int sndWriteVarMsgToDriver( struct CnetConnect* connect, struct HmiVar* pHmiVar );
    int sndReadVarMsgToDriver( struct CnetConnect* connect, struct HmiVar* pHmiVar );

    bool processRspData( struct CnetMsgPar msgPar ,struct ComMsg comMsg );
    bool trySndWriteReq( struct CnetConnect* connect );
    bool trySndReadReq( struct CnetConnect* connect );
    bool hasProcessingReq( struct CnetConnect* connect );
    bool initCommDev();

    int getComMsg( QList<struct ComMsg> &msgList );
    int onceRealUpdateVar( HmiVar* );
    bool updateOnceRealReadReq( CnetConnect* );
/*
protected:
    virtual void run();*/
signals:
...
private slots:
        ...
        void slot_run();
private:
    QList<struct CnetConnect*> m_connectList;
